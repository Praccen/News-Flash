...
export default class Doggo {
    ...
    async init() {
        let bodyMesh = await this.scene.getNewMesh("Assets/objs/body.obj", "Assets/textures/medium_fur.png", "Assets/textures/black.png");
        ...
        boundingBoxComp.boundingBox.setMinAndMaxVectors(new Vec3([-0.75, -1.5, -2.0]), new Vec3([0.75, 0.2, 2.0]));
        ...
        for (let i = -1; i < 2; i += 2) {
            for (let j = -1; j < 2; j += 2) {
                ...
                if (j == -1) {
                    let legMesh = await this.scene.getNewMesh("Assets/objs/hind_leg.obj", "Assets/textures/medium_fur.png", "Assets/textures/black.png");
                    legMesh.textureMatrix.setScale(10.0, 2.0, 1.0);
                    this.ecsManager.addComponent(this.legs[counter], new GraphicsComponent(legMesh));
                }
                else {
                    let legMesh = await this.scene.getNewMesh("Assets/objs/front_leg.obj", "Assets/textures/medium_fur.png", "Assets/textures/black.png");
                    legMesh.textureMatrix.setScale(10.0, 2.0, 1.0); 
                    this.ecsManager.addComponent(this.legs[counter], new GraphicsComponent(legMesh));
                }
                
                this.ecsManager.addComponent(this.legs[counter], new PositionComponent());
                this.ecsManager.addComponent(this.legs[counter], this.groupPositionComp);
                counter++;
            }
        }
    ...
    update(dt: number) {
        ...
        if (input.joystickDirection.length2() > 0.001) {
            ...
            accVec.subtract(new Vec3(forward).multiply(input.joystickDirection.y * 2.0));
        } 
        // Keyboard control
        else {
            if (input.keys["W"]) {
        ...
        if (accVec.length2() > 0.001) {
            ...
            let targetRotation = 90 - Math.atan2(accVec.z, accVec.x) * (180 / Math.PI);
            ...
            this.groupPositionComp.rotation.y = (this.groupPositionComp.rotation.y + (diff) * 0.04) % 360;

            let acceleration = accVec.len();

            this.movComp.accelerationDirection.x = Math.cos((90 - this.groupPositionComp.rotation.y) * (Math.PI / 180)) * Math.min(acceleration, 1.0);
            this.movComp.accelerationDirection.z = Math.sin((90 - this.groupPositionComp.rotation.y) * (Math.PI / 180)) * Math.min(acceleration, 1.0);
        ...
        if (input.keys["ARROWLEFT"]) {
            this.cameraFocusComp.offset.add(new Vec3(this.rendering.camera.getRight()).multiply(dt * 8.0));
        } else if (input.keys["ARROWRIGHT"]) {
            this.cameraFocusComp.offset.subtract(new Vec3(this.rendering.camera.getRight()).multiply(dt * 8.0));
        }
        else if (this.movComp) {
            this.cameraFocusComp.offset.subtract(new Vec3(this.movComp.velocity).multiply(dt * 3.0));
        }
        
        // ---- Keep a good camera distance ----
        let disiredDistance = 2.0;

        this.cameraFocusComp.offset.multiply(Math.pow(disiredDistance / this.cameraFocusComp.offset.len(), 0.1));
        this.cameraFocusComp.offset.y = Math.max(this.cameraFocusComp.offset.y, 1.0);
        
        let offsetXZ = new Vec3([this.cameraFocusComp.offset.x, 0.0, this.cameraFocusComp.offset.z]);
    ...
    private resetAnimation() {
        ...
        for (let i = -1; i < 2; i += 2) {
            for (let j = -1; j < 2; j += 2) {
                let legPosComp = this.legs[counter].getComponent(ComponentTypeEnum.POSITION) as PositionComponent;
                if (legPosComp) {
        ...
        let bodyPosComp = this.body.getComponent(ComponentTypeEnum.POSITION) as PositionComponent;
        if (bodyPosComp) {
    ...
    private walkAnimation(animationSpeed: number = 7.5) {
        ...
        for (let i = 0; i < this.legs.length; i++) {
            let posComp = this.legs[i].getComponent(ComponentTypeEnum.POSITION) as PositionComponent;
            if (posComp) {
                posComp.rotation.setValues(Math.sin(this.timer * animationSpeed) * 50.0 * flip[i], 0.0, 0.0);
            }
        }

        let bodyPosComp = this.body.getComponent(ComponentTypeEnum.POSITION) as PositionComponent;
        if (bodyPosComp) {
            bodyPosComp.rotation.setValues(Math.sin(this.timer * animationSpeed * 2.0) * 1.0, 0.0, Math.sin(this.timer * animationSpeed) * 6.0);
        }
    }
    ...
    private runAnimation(animationSpeed: number = 12.0) {
        ...
        for (let i = 0; i < this.legs.length; i++) {
            let posComp = this.legs[i].getComponent(ComponentTypeEnum.POSITION) as PositionComponent;
            if (posComp) {
                posComp.rotation.setValues(Math.sin(this.timer * animationSpeed + flip[i] * 0.7) * 50.0 * flip[i], 0.0, 0.0);
            }
        }

        let bodyPosComp = this.body.getComponent(ComponentTypeEnum.POSITION) as PositionComponent;
        if (bodyPosComp) {
            bodyPosComp.rotation.setValues(Math.sin(this.timer * animationSpeed) * 3.0, 0.0, Math.sin(this.timer * animationSpeed) * 5.0);
        }
    }
    ...
    private jumpAnimation(animationSpeed: number = 1.0) {
        ...
        for (let i = 0; i < this.legs.length; i++) {
            let posComp = this.legs[i].getComponent(ComponentTypeEnum.POSITION) as PositionComponent;
            if (posComp) {
                posComp.rotation.setValues(Math.sin(((Math.min(this.timer, 1.4)) + 1.1) * animationSpeed) * 80.0 * flip[i], 0.0, 0.0);
            }
        }

        let bodyPosComp = this.body.getComponent(ComponentTypeEnum.POSITION) as PositionComponent;
        if (bodyPosComp) {
            bodyPosComp.rotation.setValues(Math.sin(((Math.min(this.timer, 1.4)) + 1.1) * animationSpeed) * 5.0, 0.0, 0.0);
        }
