...
export default class CollisionSystem extends System {
    ...
    update(dt: number) {
        ...
        for (let e1 of this.entities) {
            ...
            let e1CollisionComp = <MeshCollisionComponent> e1.getComponent(ComponentTypeEnum.MESHCOLLISION);
            e1CollisionComp.updateTransformMatrix();
            if (e1CollisionComp.isStatic) {
                continue;
            }
            for (let e2 of this.entities) {
                if (e1.id == e2.id) { // Don't collide with self
                    continue;
                }

                let e2CollisionComp = <MeshCollisionComponent> e2.getComponent(ComponentTypeEnum.MESHCOLLISION);

                e2CollisionComp.updateTransformMatrix();

                IntersectionTester.identifyMeshVsMeshWithInformation(e1CollisionComp.triangles, e2CollisionComp.triangles, information);                
            }

            let movComp = <MovementComponent> e1.getComponent(ComponentTypeEnum.MOVEMENT);
            let posComp = <PositionComponent> e1.getComponent(ComponentTypeEnum.POSITION);
