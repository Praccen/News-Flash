...
export default class PointLight {
	...
	bind() {
		...
		ul = this.shaderProgram.getUniformLocation(
			"pointLights[" + this.lightIndex + "].constant"
		);
		if (ul[1]) {
			this.gl.uniform1f(ul[0], this.constant);
		}
		ul = this.shaderProgram.getUniformLocation(
			"pointLights[" + this.lightIndex + "].linear"
		);
		if (ul[1]) {
			this.gl.uniform1f(ul[0], this.linear);
		}
		ul = this.shaderProgram.getUniformLocation(
			"pointLights[" + this.lightIndex + "].quadratic"
		);
		if (ul[1]) {
