...
export default class Heightmap extends Mesh {
        ...
        for (let z = 0; z < this.zResolution; z++) {
            for (let x = 0; x < this.xResolution; x++) {
                this.updateVertexData(x, z, [
                    x * xQuadSize, 0.0, z * zQuadSize, 
                    0.0, 1.0, 0.0, 
                    x / (this.xResolution - 1), z / (this.zResolution - 1)
                ]);
            }
        }
    ...
    async readHeightDataFromTexture(texturePath: string, createResolutionFromPixels: boolean = true) {
        ...
        let resizeImage = function(image: HTMLImageElement, newWidth:number, newHeight: number): Uint8ClampedArray {
    ...
    getHeight(x: number, z: number): number {
        if (x < 0 || x > this.xResolution * this.xQuadSize ||
            z < 0 || z > this.zResolution * this.zQuadSize) {
                return null;
        }
        ...
        let topRightHeight = this.imageData[(baseX + 1) * 4 + (baseZ + 0) * this.xResolution * 4] / 255.0;
        let bottomLeftHeight = this.imageData[(baseX + 0) * 4 + (baseZ + 1) * this.xResolution * 4] / 255.0;
        let topLeftHeight = this.imageData[(baseX + 0) * 4 + (baseZ + 0) * this.xResolution * 4] / 255.0;
        let bottomRightHeight = this.imageData[(baseX + 1) * 4 + (baseZ + 1) * this.xResolution * 4] / 255.0;

        if ((baseZ + baseX) % 2 == 0) { // Because of the zig-zagging mentioned in the createPlane indices set up
            /*
            Base
            |
            v
