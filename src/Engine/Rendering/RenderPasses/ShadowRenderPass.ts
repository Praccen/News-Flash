...
import { grassShadowPass } from "../../ShaderPrograms/ShadowMapping/GrassShadowPass.js";
...
export default class ShadowRenderPass {
    ...
    draw(scene: Scene, camera: Camera) {
		...
		//Set uniforms
		scene.getDirectionalLight().calcAndSendLightSpaceMatrix(
			camera.getPosition(),
			this.shadowOffset,
		...
		gl.uniform1f(grassShadowPass.getUniformLocation("currentTime")[0],
