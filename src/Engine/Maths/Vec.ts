export default class Vec extends Array<number> {
	...
	deepAssign(base: number[]): typeof this {
		this.iterate((i: number) => {
			this[i] = base[i];
		});
	...
	subtract(vec: number[]): typeof this {
		this.iterate((i: number) => {
			this[i] -= vec[i];
		});
		return this;
	...
	multiply(mult: number): typeof this {
		this.iterate((i: number) => {
			this[i] *= mult;
		});
		return this;
