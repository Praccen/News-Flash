...
export default class Triangle3D {
    ...
    getTransformedVertices(): Array<Vec3> {
        if (this.verticesNeedsUpdate) {
            ...
            for (const originalVertex of this.originalVertices) {
                let transformedVertex = this.transformMatrix.multiplyVector4(new Vector4([originalVertex.x, originalVertex.y, originalVertex.z, 1.0]));
                let transformedVertexVec3 = new Vec3({x: transformedVertex.elements[0], y: transformedVertex.elements[1], z: transformedVertex.elements[2]});
    ...
    getTransformedNormal(): Vec3 {
        if (this.normalNeedsUpdate) {
            ...
            let transformedNormal = tempMatrix.invert().transpose().multiplyVector3(new Vector3([this.originalNormal.x, this.originalNormal.y, this.originalNormal.z])).normalize();
            this.transformedNormal = new Vec3({x: transformedNormal.elements[0], y: transformedNormal.elements[1], z: transformedNormal.elements[2]});
            this.normalNeedsUpdate = false;
        }
    ...
    getTransformedEdges(): Array<Vec3> {
        if (this.edgesNeedsUpdate) {
            ...
            this.transformedEdges.push(new Vec3(this.transformedVertices[1]).subtract(this.transformedVertices[0]).normalize());
            this.transformedEdges.push(new Vec3(this.transformedVertices[2]).subtract(this.transformedVertices[1]).normalize());
            this.transformedEdges.push(new Vec3(this.transformedVertices[0]).subtract(this.transformedVertices[2]).normalize());
    ...
    getTransformedEdgeNormals(): Array<Vec3> {
        if (this.edgeNormalsNeedsUpdate) {
            ...
            this.transformedEdgeNormals.push(new Vec3(this.transformedEdges[0]).cross(this.transformedNormal).normalize());
            this.transformedEdgeNormals.push(new Vec3(this.transformedEdges[1]).cross(this.transformedNormal).normalize());
            this.transformedEdgeNormals.push(new Vec3(this.transformedEdges[2]).cross(this.transformedNormal).normalize());
